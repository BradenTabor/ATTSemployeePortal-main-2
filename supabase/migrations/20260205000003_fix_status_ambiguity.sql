-- =============================================================================
-- Fix: Resolve "status" column ambiguity in submit_certification_test
-- The RETURNS TABLE has a "status" column which conflicts with table columns
-- =============================================================================
...
RETURNS TABLE (
  ...
  result_status TEXT  -- Renamed from 'status' to avoid ambiguity
)
LANGUAGE plpgsql
SECURITY DEFINER
...
COMMENT ON FUNCTION public.submit_certification_test(UUID, JSONB) IS
  'Grade MC/TF auto, mark short_answer as pending_review for admin grading. Fixed status column ambiguity.';
