-- Seed weekly_attendance_summary recipients from compliance_summary (separate migration so enum value is committed).
INSERT INTO public.email_recipient_lists (list_key, email, created_at)
SELECT
  'weekly_attendance_summary'::public.email_list_key,
  email,
  now()
