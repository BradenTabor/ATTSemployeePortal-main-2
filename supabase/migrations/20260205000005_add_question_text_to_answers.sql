-- =============================================================================
-- Fix: Include question_text in stored answers for admin grading
-- Also update existing pending submissions to include question text
-- =============================================================================

-- Must drop first to change function
...
-- =============================================================================
-- Backfill existing pending submissions with question text
-- =============================================================================
UPDATE public.certification_attempts ca
SET answers = (
  SELECT jsonb_agg(
    CASE 
      WHEN q.question_text IS NOT NULL THEN
        a || jsonb_build_object('question_text', q.question_text)
      ELSE a
    END
  )
  FROM jsonb_array_elements(ca.answers) a
  LEFT JOIN public.certification_questions q ON q.id = (a->>'question_id')::uuid
)
WHERE ca.status = 'submitted'
  AND ca.answers IS NOT NULL
  AND jsonb_array_length(ca.answers) > 0;

COMMENT ON FUNCTION public.submit_certification_test(UUID, JSONB) IS
  'Grade MC/TF auto, mark short_answer for review. Now includes question_text in stored answers.';
