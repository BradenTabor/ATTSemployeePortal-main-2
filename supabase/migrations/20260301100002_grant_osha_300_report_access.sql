/*
  Grant execute on get_incident_log_osha_300_301 so admin/supervisor users can
  view and export the OSHA 300 report. Add a role check inside the function so
  only admin or supervisor can run it (SECURITY DEFINER returns all rows).
*/

-- Allow authenticated users to call the RPC (function enforces admin/supervisor)
GRANT EXECUTE ON FUNCTION public.get_incident_log_osha_300_301(date, date) TO authenticated;

-- Recreate function with permission check: only admin or supervisor can run it
...
AS $$
BEGIN
  -- Only admin or supervisor can view OSHA 300 report (SECURITY DEFINER returns all incidents)
  IF NOT (public.is_admin() OR public.is_supervisor()) THEN
    RAISE EXCEPTION 'Permission denied: OSHA 300 report requires admin or supervisor role.'
...
COMMENT ON FUNCTION public.get_incident_log_osha_300_301(date, date) IS
  'Returns incident log with OSHA 300/301 fields and job/crew/supervisor traceability. Callable by admin or supervisor only.';
