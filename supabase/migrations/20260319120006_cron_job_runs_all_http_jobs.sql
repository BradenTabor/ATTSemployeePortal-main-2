-- Include all HTTP-invoked cron jobs (those that call Edge Functions with service role auth)
-- in the monitoring view and failure helper. Matches the jobs updated by scripts/deploy-cron-auth.sh.
...
COMMENT ON VIEW public.cron_job_runs IS
  'Monitoring view for all HTTP cron jobs (Edge Function invocations). Use deploy-cron-auth.sh or Vault (5am only) for auth.';
