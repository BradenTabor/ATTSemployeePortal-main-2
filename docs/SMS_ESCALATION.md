...
- **STOP opt-out:** All escalation SMS include "Reply STOP to opt out." Enforcement is **application-side**: recipient selection filters on `app_users.sms_operational_opt_out`, gated by the `app_settings.sms_send_optout_filter_config` kill switch (default ON). Every excluded recipient is recorded in `sms_escalation_send_log.suppression_log`; an opted-out Tier 2 static recipient is skipped with a loud warning, and a Tier 2 list emptied by the filter logs at error level and still writes an audit row.
  > **Corrected 2026-09-09.** This line previously read *"ClickSend handles STOP at the carrier level automatically — no application-side webhook needed."* That is false. ClickSend's opt-out list is only consulted for sends addressed to a contact list; the portal sends ad-hoc to a raw `to` number, so the carrier never suppresses an opted-out recipient. Delivery receipts proved it: a number on ClickSend's opt-out list since 2026-03-04 received 530 further messages. The webhook (`clicksend-inbound-webhook`) sets the flags; the send-path filter is what actually stops the send. See `docs/sms-upgrade/13-UNREACHABLE-CREW.md`.
