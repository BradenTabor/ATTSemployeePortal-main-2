...
## Session 21 — 2026-09-02

### App update pipeline rebuild (bug: post-deploy "new version" countdown + reload spam blocked sign-in)
- **Root cause**: two competing updaters. `DeployVersionChecker` reloaded whenever `/version.json` disagreed with the running build while the old service worker kept serving the old precached shell, so each reload came back old and re-triggered; its guard `clearReloadAttempts()` ran on every mount so it never tripped. `RequiredUpdatePrompt` showed a mandatory modal at the same time. `vercel.json` header order also let the catch-all `no-cache` beat `immutable` for hashed assets.
- **Replaced with** `src/lib/appUpdate/*` (single `AppUpdateController` state machine, Zustand store, sessionStorage reload guard: 1 auto reload per target build / 2 per 10 min, pure `versionCheck`, `safeRoutes`, SW adapter) + `useAppUpdate` + `AppUpdatePrompt` (non-blocking banner, 10 s countdown only on safe routes with an empty offline queue, Later = 30 min snooze, blocking overlay only for stale-chunk failures, "Updated to version X" pill after the reload). The page reloads in exactly one place — after `controllerchange` — and a reload that lands on the old build stops with an explanation instead of retrying. Sign-in page counts as unsafe for the mid-session countdown; launch-time applies still happen before the first tap.
- **SW**: navigation now served by `createHandlerBoundToURL('index.html')` with a denylist (`sw.js`, `version.json`, `manifest.json`, `/assets/`, APIs); no auto `skipWaiting`. **vercel.json**: rule order fixed (`/assets/*` immutable; `sw.js`/`index.html`/`version.json` no-store).
- **Deleted**: `DeployVersionChecker.tsx`, `checkDeployVersion.ts`, `RequiredUpdatePrompt.tsx`, `PWAUpdatePrompt.tsx`.
- **Diagnostics**: `window.__ATTS_UPDATE__` (`version`, `buildTime`, `state()`, `checkForUpdates()`), logs prefixed `[AppUpdate]`.
- **Tests**: 61 unit tests (`tests/unit/lib/appUpdate/*`, controller driven by a fake SW adapter under fake timers — includes the exact incident: server newer forever → zero reloads), `tests/e2e/app-update.spec.ts` (newer version.json → ≤1 load, sign-in usable, no overlay). Pass 2 ran a real SW update A→B→A in the browser against a Vercel-like static server (desktop + 390 px): one reload each time, banner/countdown/Later/relaunch-apply/pill all verified.
- **Docs**: `docs/APP_UPDATE_PIPELINE.md` (architecture, policy table, pre-flight checklist), `offline-manual-test-scripts.md` Test 4 rewritten.
- Rollback: `git checkout -- src/App.tsx src/main.tsx src/sw.ts vercel.json src/components/AppNotificationShell.tsx src/components/notifications/index.ts && git checkout -- src/components/DeployVersionChecker.tsx src/lib/checkDeployVersion.ts src/components/notifications/RequiredUpdatePrompt.tsx src/components/notifications/PWAUpdatePrompt.tsx && rm -rf src/lib/appUpdate src/init/app-update-init.ts src/hooks/useAppUpdate.ts src/components/notifications/AppUpdatePrompt.tsx tests/unit/lib/appUpdate tests/e2e/app-update.spec.ts`.
