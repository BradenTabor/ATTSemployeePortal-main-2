...
/** Photo thumbnails shown in review step when paper JSA images are attached. */
function PaperJsaPhotoReview({ paths, onEdit }: { paths: string[]; onEdit: () => void }) {
  // Store the paths key alongside urls so we can derive loading when paths change
  ...
  // Loading when we have no result yet or the result is for stale paths
    ...
    <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 overflow-hidden">
      ...
      <div className="px-3 py-2">
          ...
          <div className="flex flex-wrap gap-2">
            {paths.map((path) => {
              ...
              return (
                <div
                  key={path}
                  className="rounded-lg overflow-hidden border border-white/10 bg-black/40 w-16 h-16 sm:w-20 sm:h-20 flex-shrink-0"
                >
                  {url ? (
                    <img
                      src={url}
        ...
        <p className="text-[10px] text-white/30 mt-1.5">
          These photos will be included when this JSA is exported.
        </p>
      </div>
    </div>
  );
...
export function StepReview({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Paper JSA Photos (only shown if photos exist) */}
      {form.jsaPhotoPaths && form.jsaPhotoPaths.length > 0 && (
        <PaperJsaPhotoReview paths={form.jsaPhotoPaths} onEdit={() => onGoToStep(1)} />
      )}
