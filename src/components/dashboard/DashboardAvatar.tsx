...
const VARIANT_COLORS: Record<AvatarVariant, { primary: string; secondary: string; icon: string }> = {
  ...
  jobs: { primary: colors.safety.orange, secondary: '#7CC43F', icon: '#ffffff' },
...
function SimplifiedAvatar({ variant }: { variant: AvatarVariant }) {
  ...
  return (
    ...
    >
      ...
      {/* Legs - simplified */}
      <rect x="29" y="68" width="8" height="26" rx="3" fill="#2F3F36" />
      <rect x="43" y="68" width="8" height="26" rx="3" fill="#2F3F36" />
      ...
      {/* Reflective stripes */}
      <rect x="26" y="50" width="8" height="2" rx="0.5" fill="#FBFFE8" />
      <rect x="26" y="56" width="8" height="2" rx="0.5" fill="#FBFFE8" />
      <rect x="46" y="50" width="8" height="2" rx="0.5" fill="#FBFFE8" />
      <rect x="46" y="56" width="8" height="2" rx="0.5" fill="#FBFFE8" />
      ...
      <circle cx="35.5" cy="26.5" r="2" fill="#121A15" />
      <circle cx="44.5" cy="26.5" r="2" fill="#121A15" />
      <circle cx="35" cy="25.5" r="0.8" fill="white" />
      <circle cx="44" cy="25.5" r="0.8" fill="white" />

      {/* Smile */}
      <path d="M 35 33 Q 40 37 45 33" stroke="#8DB52A" strokeWidth="1.5" fill="none" strokeLinecap="round" />
      ...
      {/* ATTS text */}
      <text x="40" y="15" fontSize="4" fill="#1F7A44" textAnchor="middle" fontWeight="bold">ATTS</text>
      ...
      {variant === 'jobs' && (
        <g transform="translate(58, 55)">
          ...
          <rect x="12" y="2" width="8" height="4" fill="#8A9A8E" />
        </g>
      )}
      {variant === 'tools' && (
        <g transform="translate(60, 15) rotate(25)">
          <rect x="-2" y="0" width="4" height="18" rx="1" fill="#8A9A8E" />
          <path d="M -4 -2 Q -5 -5 -2 -6 L 2 -6 Q 5 -5 4 -2 L 2 0 L -2 0 Z" fill="#5A6B60" />
        </g>
      )}
  ...
  return (
    ...
    >
      <defs>
        ...
        <linearGradient id={`${id}-skin`} x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="0%" stopColor="#ECFFAE" />
          <stop offset="25%" stopColor="#E4FFC2" />
          <stop offset="50%" stopColor="#AEDB3F" />
          <stop offset="75%" stopColor="#8A9A8E" />
          <stop offset="100%" stopColor="#8A9A8E" />
        </linearGradient>

        <radialGradient id={`${id}-skin-face`} cx="50%" cy="35%" r="65%">
          <stop offset="0%" stopColor="#ECFFAE" />
          <stop offset="30%" stopColor="#E4FFC2" />
          <stop offset="60%" stopColor="#AEDB3F" />
          <stop offset="100%" stopColor="#8A9A8E" />
        </radialGradient>

        {/* Hardhat gradients */}
        <linearGradient id={`${id}-hardhat`} x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="0%" stopColor="#DDFF85" />
          <stop offset="15%" stopColor="#E4FFC2" />
          <stop offset="40%" stopColor="#D2FFA3" />
          <stop offset="70%" stopColor="#8DB52A" />
          <stop offset="100%" stopColor="#6B8A1F" />
        </linearGradient>

        {/* Safety vest */}
        <linearGradient id={`${id}-vest`} x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="0%" stopColor="#B8FF7A" />
          <stop offset="20%" stopColor="#9BEB5B" />
          <stop offset="60%" stopColor="#7CC43F" />
          <stop offset="100%" stopColor="#5E9A2C" />
        </linearGradient>

        <linearGradient id={`${id}-vest-stripe`} x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="0%" stopColor="#FBFFE8" />
          <stop offset="50%" stopColor="#FBFFE8" />
          <stop offset="100%" stopColor="#DDFF85" />
        </linearGradient>

        {/* Work shirt */}
        <linearGradient id={`${id}-shirt`} x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="0%" stopColor="#ECFFAE" />
          <stop offset="30%" stopColor="#DDFF85" />
          <stop offset="100%" stopColor="#8A9A8E" />
        </linearGradient>

        {/* Pants */}
        <linearGradient id={`${id}-pants`} x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="0%" stopColor="#5A6B60" />
        ...
        <linearGradient id={`${id}-leather`} x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="0%" stopColor="#6B8A1F" />
          <stop offset="50%" stopColor="#4A6116" />
          <stop offset="100%" stopColor="#1E2A23" />
        </linearGradient>
      ...
      <g>
        ...
        {/* Zipper */}
        <rect x="39" y="44" width="2" height="24" fill="#5A6B60" />
        
        {/* Belt */}
        <rect x="24" y="66" width="32" height="4" rx="1" fill="#2F3F36" />
        ...
        {variant === 'announcements' && (
          <>
            ...
            <g transform="translate(5, 61) rotate(-15)">
              <rect x="0" y="0" width="11" height="15" rx="1" fill="#5A6B60" />
              <rect x="1" y="2" width="9" height="11" rx="0.5" fill="#F4F7F2" />
            </g>
        ...
        {variant === 'jobs' && (
          <>
            ...
            <g transform="translate(9, 57)">
              <ellipse cx="4" cy="3" rx="5" ry="3" fill="none" stroke="#8A9A8E" strokeWidth="2" />
            </g>
            ...
            <g>
              <rect x="61" y="55" width="18" height="12" rx="2" fill="#9BEB5B" />
        ...
        {variant === 'tools' && (
          <>
            ...
            <g transform="translate(-1, 61)">
              ...
              <path d="M 6 0 Q 6 -3 10 -3 Q 14 -3 14 0" stroke="#121A15" strokeWidth="2" fill="none" />
            </g>
            ...
            <g transform="translate(64, 6) rotate(25)">
              ...
              <path d="M -5 -2 Q -6 -5 -3 -7 L 3 -7 Q 6 -5 5 -2 L 3 0 L -3 0 Z" fill="#5A6B60" />
            </g>
          </>
        )}
      </g>
      ...
      <g>
        ...
        {/* Ears */}
        <ellipse cx="26" cy="28" rx="3" ry="4.5" fill="#E4FFC2" />
        <ellipse cx="54" cy="28" rx="3" ry="4.5" fill="#E4FFC2" />
        ...
        {/* Eyebrows */}
        <path d="M 31 20 Q 35 17.5 39 20" stroke="#2F3F36" strokeWidth="1.5" fill="none" strokeLinecap="round" />
        <path d="M 41 20 Q 45 17.5 49 20" stroke="#2F3F36" strokeWidth="1.5" fill="none" strokeLinecap="round" />

        {/* Eyes */}
        <ellipse cx="35" cy="25" rx="4.5" ry="5" fill="white" />
        <ellipse cx="36" cy="25.5" rx="2.8" ry="3.2" fill={`url(#${id}-iris)`} />
        <ellipse cx="36" cy="26" rx="1.5" ry="2" fill="#0B100D" />
        ...
        <ellipse cx="44" cy="26" rx="1.5" ry="2" fill="#0B100D" />
        <circle cx="43" cy="24" r="1.2" fill="white" />

        {/* Nose */}
        <ellipse cx="40" cy="30" rx="2.5" ry="2" fill="#AEDB3F" opacity="0.4" />

        {/* Mouth */}
        <path d="M 34 34 Q 40 39 46 34" stroke="#8DB52A" strokeWidth="1.8" strokeLinecap="round" fill="none" />
        ...
        <rect x="35" y="19" width="10" height="4" rx="1.5" fill="#5A6B60" />
        
        {/* ATTS text */}
        <text x="40" y="15" fontSize="4.5" fill="#1F7A44" textAnchor="middle" fontWeight="bold">ATTS</text>

        {/* Safety glasses (not jobs variant) */}
        {variant !== 'jobs' && (
          <g>
            <path d="M 27 17 L 53 17" stroke="#2FA45A" strokeWidth="1.5" />
            <ellipse cx="32" cy="17.5" rx="6" ry="2.5" fill="#3DDC84" opacity="0.25" stroke="#2FA45A" strokeWidth="0.8" />
            <ellipse cx="48" cy="17.5" rx="6" ry="2.5" fill="#3DDC84" opacity="0.25" stroke="#2FA45A" strokeWidth="0.8" />
          </g>
        )}

        {/* Visor for jobs variant */}
        {variant === 'jobs' && (
          <path d="M 25 21 Q 25 14 40 14 Q 55 14 55 21 L 53 26 Q 40 29 27 26 Z" fill="#2FA45A" opacity="0.35" />
        )}
      </g>
    </svg>
  );
