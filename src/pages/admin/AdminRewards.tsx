...
const MobileUserCard = memo(({ user, index, onClick }: UserRewardCardProps) => (
  ...
  >
    <div className="rounded-xl sm:rounded-2xl border border-[#f6dcb2]/15 bg-gradient-to-br from-[#1b1914]/80 to-[#120f0c]/60 p-3 sm:p-4 active:scale-[0.98] active:bg-[#f4c979]/5 transition-all">
      <div className="flex items-start gap-2.5 sm:gap-3">
        {/* Avatar */}
        <div className="w-9 h-9 sm:w-11 sm:h-11 rounded-lg sm:rounded-xl bg-gradient-to-br from-[#f4c979] to-[#d89d3e] flex items-center justify-center text-[#2d1c04] font-semibold text-sm sm:text-base flex-shrink-0">
          {(user.full_name || user.email || "?").charAt(0).toUpperCase()}
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10px] sm:text-xs text-[#f4c979]/80 truncate">
            {user.email || "No email"}
          </p>
        </div>

        {/* Points badge */}
        <div className="flex items-center gap-1 px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-lg sm:rounded-xl bg-[#f4c979]/20 border border-[#f4c979]/40 flex-shrink-0">
          <Star className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-[#f4c979]" />
          <span className="text-xs sm:text-sm font-bold text-[#fef3d1]">{user.total_points}</span>
        </div>
      </div>
      ...
      <div className="mt-2.5 sm:mt-3 pt-2.5 sm:pt-3 border-t border-white/5 flex items-center justify-between">
        ...
        <span className="inline-flex items-center gap-1 text-[10px] sm:text-xs text-[#f4c979]/70">
          View
...
const StatCard = ({ icon, label, value, subtext }: StatCardProps) => (
  <motion.div
    ...
    className="rounded-xl sm:rounded-2xl border border-[#f6dcb2]/20 bg-gradient-to-br from-[#1b1914]/80 to-[#120f0c]/60 p-3 sm:p-4 flex items-center gap-2.5 sm:gap-4"
  >
    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl bg-[#f4c979]/15 border border-[#f4c979]/30 flex items-center justify-center flex-shrink-0 [&>svg]:w-5 [&>svg]:h-5 sm:[&>svg]:w-6 sm:[&>svg]:h-6">
      {icon}
    </div>
    <div className="min-w-0">
      <p className="text-[9px] sm:text-xs uppercase tracking-wider text-[#f4c979]/70 truncate">{label}</p>
      <p className="text-xl sm:text-2xl font-bold text-white">{value}</p>
      {subtext && <p className="text-[9px] sm:text-xs text-[#c7b696] truncate hidden xs:block">{subtext}</p>}
    </div>
  </motion.div>
);
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 pb-4 pt-4 sm:pt-6">
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-4">
            <StatCard
              icon={<Gift className="text-[#f4c979]" />}
              label="Claims"
              value={stats?.totalClaims ?? 0}
              subtext="All-time rewards claimed"
            />
            <StatCard
              icon={<TrendingUp className="text-[#f4c979]" />}
              label="Points"
              value={stats?.totalPoints ?? 0}
              subtext="Points awarded to users"
            />
            <StatCard
              icon={<Users className="text-[#f4c979]" />}
              label="Users"
          ...
          <motion.div
            ...
            className="rounded-2xl sm:rounded-3xl border border-[#f6dcb2]/20 bg-gradient-to-br from-[#1b1914] via-[#120f0c] to-[#080705] p-3 sm:p-5 shadow-[0_25px_50px_rgba(0,0,0,0.55)]"
          >
