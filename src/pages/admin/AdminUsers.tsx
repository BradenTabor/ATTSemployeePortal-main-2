...
const getExperienceBadgeClass = (level: string | null): string => {
  switch (level) {
    case 'expert':
      return 'bg-[#121A15] text-[#8DF5A8] border border-emerald-500/40';
    case 'journeyman':
      return 'bg-[#1C3B2C] text-[#B4E6C9] border border-indigo-400/40';
    case 'apprentice':
      return 'bg-[#1E2A23] text-[#C8F55E] border border-amber-400/40';
    default:
      return 'bg-white/5 text-[#8A9A8E] border border-white/10';
  }
};
...
const MobileUserCard = ({
  ...
  return (
    <motion.article
      ...
      className="rounded-xl sm:rounded-leaf-sm border border-[#E4EAE1]/15 bg-gradient-to-br from-[#121A15]/80 to-[#0B100D]/60 p-3 sm:p-4 space-y-2.5 sm:space-y-3 active:bg-[#F4F7F2]/5 transition-colors"
    >
      {/* User Avatar & Email */}
      <div className="flex items-start gap-2.5 sm:gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {user.full_name && (
            <p className="text-[9px] sm:text-[0.65rem] text-[#B8C4B6] truncate">{user.email}</p>
          )}
          <p className="text-[9px] sm:text-[0.65rem] text-[#B8C4B6]/60">ID: {user.id.slice(0, 8)}…</p>
        </div>
      </div>

      {/* Role Section */}
      <div className="space-y-1.5 sm:space-y-2">
        <p className="text-[9px] sm:text-xs uppercase text-[#F4F7F2]/70 font-mono font-medium tracking-[0.14em]">Role</p>
        ...
        {editingRoleUserId === user.id ? (
          <div className="space-y-2" data-testid="edit-user-form">
            ...
            <select
              ...
              disabled={savingRole}
              className="w-full rounded-lg sm:rounded-xl bg-[#040605]/70 border border-[#F4F7F2]/20 px-2.5 sm:px-3 py-2 text-xs sm:text-sm text-[#F4F7F2] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#F4F7F2]/60 min-h-[40px] sm:min-h-[44px]"
            >
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={savingRole}
                className="flex-1 px-2.5 sm:px-3 py-2 rounded-lg sm:rounded-xl bg-[#F4F7F2]/20 border border-[#F4F7F2]/40 text-[10px] sm:text-xs font-semibold text-[#F4F7F2] hover:bg-[#F4F7F2]/30 active:bg-[#F4F7F2]/40 disabled:opacity-50 transition-colors min-h-[40px] sm:min-h-[44px]"
              >
              ...
              <button
                ...
                disabled={savingRole}
                className="flex-1 px-2.5 sm:px-3 py-2 rounded-lg sm:rounded-xl border border-[#E4EAE1]/25 text-[10px] sm:text-xs font-semibold text-[#F4F7F2] hover:bg-white/5 active:bg-white/10 disabled:opacity-50 transition-colors min-h-[40px] sm:min-h-[44px]"
              >
                Cancel
              </button>
            </div>
          ...
          <div className="flex items-center justify-between gap-2 flex-wrap">
            ...
            <button
              ...
              className="tap-44 relative px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg sm:rounded-xl border border-[#E4EAE1]/25 text-[10px] sm:text-xs font-semibold text-[#F4F7F2] hover:bg-white/5 active:bg-white/10 transition-colors min-h-[36px] sm:min-h-[40px]"
            >
              Edit
            </button>
          </div>
      ...
      {/* Manager Section */}
      <div className="space-y-1.5 sm:space-y-2">
        <p className="text-[9px] sm:text-xs uppercase text-[#F4F7F2]/70 font-mono font-medium tracking-[0.14em]">Manager</p>
        <div className="flex items-center justify-between gap-2">
          <span className="text-[10px] text-[#D3DCD1]">
            {user.manager_id ? (managerNameMap[user.manager_id] ?? "—") : "—"}
          </span>
          <button
            onClick={() => onEditManager(user)}
            className="tap-44 relative px-2.5 py-1.5 rounded-lg border border-[#E4EAE1]/25 text-[10px] font-semibold text-[#F4F7F2] hover:bg-white/5 transition-colors"
          >
      ...
      {/* Experience Section */}
      <div className="space-y-1.5 sm:space-y-2">
        <p className="text-[9px] sm:text-xs uppercase text-[#F4F7F2]/70 font-mono font-medium tracking-[0.14em]">Experience</p>
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 flex-wrap">
            {tenure && (
              <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-semibold ${
                tenure.isNewHire 
                  ...
                  : 'bg-[#1E2A23] text-[#8DF5A8] border border-green-500/30'
              }`}>
            ...
            {!tenure && !user.experience_level && (
              <span className="text-[10px] text-[#B8C4B6]/60">Not set</span>
            )}
          </div>
          <button
      ...
      {canAct && (
        <div className="space-y-1.5 sm:space-y-2">
          <p className="text-[9px] sm:text-xs uppercase text-[#F4F7F2]/70 font-mono font-medium tracking-[0.14em]">Actions</p>
          <div className="flex flex-wrap gap-2">
            {user.status === 'blocked' ? (
              <button
                onClick={() => onUnblock(user)}
                disabled={unblockPending}
                className="tap-44 relative inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-emerald-500/30 text-[10px] font-semibold text-emerald-300 hover:bg-emerald-500/10 disabled:opacity-50"
              >
              ...
              <button
                ...
                className="tap-44 relative inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-amber-500/30 text-[10px] font-semibold text-amber-300 hover:bg-amber-500/10"
              >
            ...
            <button
              ...
              className="tap-44 relative inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-red-500/30 text-[10px] font-semibold text-red-300 hover:bg-red-500/10"
            >
      ...
      <div className="pt-1.5 sm:pt-2 border-t border-white/5">
        <p className="text-[10px] sm:text-xs text-[#D3DCD1]">{formatJoinedDate(user.created_at)}</p>
        <p className="text-[9px] sm:text-[0.7rem] text-[#B8C4B6]">{formatJoinedTime(user.created_at)}</p>
      </div>
    </motion.article>
  );
};
...
const MobileLoadingSkeleton = () => (
  <div className="md:hidden space-y-3 p-4">
    {Array.from({ length: 4 }).map((_, index) => (
      <div
        key={index}
        className="rounded-leaf-sm border border-white/5 bg-white/5 h-28 animate-pulse"
      />
    ))}
  </div>
);
    ...
    <AnimatePresence>
      ...
      >
        ...
        <motion.div
          ...
          className="relative w-full max-w-md max-h-[min(90vh,32rem)] overflow-y-auto rounded-leaf-sm border border-[#E4EAE1]/20 bg-gradient-to-br from-[#121A15] via-[#0B100D] to-[#040605] p-6 shadow-2xl"
        >
            {/* Header */}
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-[#F4F7F2]/15 border border-[#F4F7F2]/30 flex items-center justify-center">
                  <Award className="w-5 h-5 text-[#F4F7F2]" />
                </div>
                <div>
                  <h3 className="text-lg font-bold text-white">Edit Experience</h3>
                  <p className="text-xs text-[#B8C4B6]">{user.full_name || user.email}</p>
                </div>
              </div>
              <button
                onClick={onClose}
              ...
              >
                <X className="w-5 h-5 text-[#B8C4B6]" />
              </button>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-xl bg-[#040605]/70 border border-[#F4F7F2]/20 px-4 py-3 text-sm text-[#F4F7F2] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#F4F7F2]/60"
                />
                {hireDate && (
                  <p className="mt-1.5 text-xs text-[#B8C4B6]">
            ...
            <div className="flex gap-3 mt-6">
              <button
                onClick={onClose}
                disabled={saving}
                className="flex-1 px-4 py-3 rounded-xl border border-[#E4EAE1]/25 text-sm font-semibold text-[#F4F7F2] hover:bg-white/5 disabled:opacity-50 transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={handleSave}
                disabled={saving}
                className="flex-1 px-4 py-3 rounded-xl bg-[#F4F7F2]/20 border border-[#F4F7F2]/40 text-sm font-semibold text-[#F4F7F2] hover:bg-[#F4F7F2]/30 disabled:opacity-50 transition-colors"
              >
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-[#1E2A23] border border-[#3DDC84]/40 flex items-center justify-center">
                  <UserCog className="w-5 h-5 text-[#8DF5A8]" />
                </div>
                <div>
            ...
            <div className="space-y-4">
              <div>
                ...
                {loadingPicker ? (
                  <p className="text-sm text-[#B8C4B6]">Loading users…</p>
                ) : (
                  <select
                    value={managerId}
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          <p className="text-sm text-[#B8C4B6] mb-4">
            This will prevent the user from logging in. Their data will be preserved and the account can be unblocked later.
          </p>
          <div>
            <label className="block text-xs text-[#F4F7F2]/80 uppercase mb-2 font-mono font-medium tracking-[0.14em]">Reason (optional)</label>
            <textarea
              value={reason}
              onChange={(e) => setReason(e.target.value)}
              placeholder="e.g. Policy violation, Suspicious activity..."
              rows={3}
              className="w-full rounded-xl bg-[#040605]/70 border border-[#F4F7F2]/20 px-4 py-3 text-sm text-[#F4F7F2] placeholder:text-[#5A6B60] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#F4F7F2]/60 resize-none"
            />
          </div>
          <div className="flex gap-3 mt-6">
            <button
              onClick={onClose}
              disabled={pending}
              className="flex-1 px-4 py-3 rounded-xl border border-[#E4EAE1]/25 text-sm font-semibold text-[#F4F7F2] hover:bg-white/5 disabled:opacity-50"
            >
              Cancel
            </button>
            <button
    ...
    <AnimatePresence>
      ...
      >
        ...
        <motion.div
          ...
          transition={{ duration: 0.2, ease: [0.22, 1, 0.36, 1] }}
          onClick={(e) => e.stopPropagation()}
          className="relative w-full max-w-md max-h-[min(90vh,32rem)] overflow-y-auto rounded-leaf-sm border border-red-500/30 bg-gradient-to-br from-[#121A15] via-[#0B100D] to-[#040605] p-6 shadow-2xl"
        >
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-[#B8C4B6] mt-0.5">{user.email}</p>
              </div>
            </div>
            <button onClick={onClose} className="p-2 rounded-lg hover:bg-white/5" aria-label="Close">
              <X className="w-5 h-5 text-[#B8C4B6]" />
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-xs text-[#F4F7F2]/80 uppercase mb-2 font-mono font-medium tracking-[0.14em]">Reason (required)</label>
              <textarea
                value={reason}
                onChange={(e) => setReason(e.target.value)}
            ...
            <div>
              <label className="block text-xs text-[#F4F7F2]/80 uppercase mb-2 font-mono font-medium tracking-[0.14em]">
                Type <span className="text-[#B8C4B6] font-normal select-all" title="Select to copy">{DELETE_CONFIRM_PHRASE}</span> to confirm
                <button
                  type="button"
                  onClick={handleCopyPhrase}
                  className="ml-2 px-2 py-0.5 rounded text-[10px] font-medium border border-[#F4F7F2]/30 text-[#B8C4B6] hover:bg-white/5 hover:border-[#F4F7F2]/50"
                >
                  Copy
                </button>
              </label>
              <input
                ...
                className="w-full rounded-xl bg-[#040605]/70 border border-[#F4F7F2]/20 px-4 py-3 text-sm text-[#F4F7F2] placeholder:text-[#5A6B60] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#F4F7F2]/60"
                spellCheck={false}
              />
            </div>
          </div>
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 pb-4 pt-4 sm:pt-6">
              ...
              <div className="relative px-5 py-4 md:px-7 md:py-5">
                <div className="flex items-center gap-3 mb-3 flex-wrap">
                  <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4, delay: 0.2 }} className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-[#F4F7F2]/15 border border-[#F4F7F2]/30">
                    ...
                    <span className="text-[10px] uppercase text-[#E4EAE1] font-mono font-medium tracking-[0.14em]">Admin • Directory</span>
                  </motion.div>
                  <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4, delay: 0.3 }} className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-[#121A15]/60 border border-[#F4F7F2]/20">
                    ...
                    <span className="text-[9px] uppercase text-[#E4EAE1]/70 font-mono font-medium tracking-[0.14em]">{debouncedSearchQuery ? "Filtered" : "Live"} view</span>
                  </motion.div>
                  <Link
                    to="/admin/email-recipients"
                    className="tap-44 relative inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-[#F4F7F2]/30 bg-[#F4F7F2]/10 text-[10px] font-semibold text-[#E4EAE1] hover:bg-[#F4F7F2]/20 hover:border-[#F4F7F2]/50 transition-colors"
                  >
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    {enableAnimations ? (
                      <TextEffect as={embedded ? "h2" : "h1"} preset="blurSlide" per="char" delay={0.15} className="type-display font-light text-bone-50 text-[clamp(1.6rem,3.8vw,2.6rem)]" segmentWrapperClassName="text-glow">
                        User Management Console
                      </TextEffect>
                    ) : embedded ? (
                      <h2 className="type-display font-light text-bone-50 text-[clamp(1.6rem,3.8vw,2.6rem)]">User Management Console</h2>
                    ) : (
                      <h1 className="type-display font-light text-bone-50 text-[clamp(1.6rem,3.8vw,2.6rem)]">User Management Console</h1>
                    )}
        ...
        <div className="space-y-4 sm:space-y-6 md:space-y-8">
          <motion.div
            ...
            className="rounded-leaf-sm sm:rounded-leaf border border-[#E4EAE1]/20 bg-gradient-to-br from-[#121A15] via-[#0B100D] to-[#040605] p-3 sm:p-4 md:p-6 space-y-3 sm:space-y-4 shadow-[0_25px_50px_rgba(0,0,0,0.55)]"
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4">
              ...
              <div className="relative">
                ...
                <select
                  ...
                  className="w-full rounded-xl sm:rounded-leaf-sm bg-[#040605]/70 border border-[#F4F7F2]/20 pl-9 sm:pl-11 pr-8 sm:pr-4 py-2.5 sm:py-3 text-xs sm:text-sm text-[#F4F7F2] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#F4F7F2]/60 appearance-none cursor-pointer min-h-[42px] sm:min-h-[48px]"
                >
            ...
            {(searchQuery || roleFilter) && (
              ...
              >
                ...
                {roleFilter && (
                  <span className="inline-flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1 rounded-full border border-[#E4EAE1]/30 bg-[#E4EAE1]/10 text-[10px] sm:text-xs text-[#F4F7F2]">
            ...
            {loading ? (
              ...
              <div className="text-center py-24 space-y-3">
                <div className="inline-flex items-center justify-center w-16 h-16 rounded-leaf-sm bg-[#121A15] border border-[#E4EAE1]/30 mx-auto">
                  <Users className="w-7 h-7 text-[#F4F7F2]" />
                </div>
              ...
              <>
                <div className="hidden md:block overflow-x-auto">
                  <table className="w-full" data-testid="user-list">
                    <thead className="bg-gradient-to-r from-[#1E2A23] to-[#121A15] border-b border-[#E4EAE1]/15 text-[0.65rem] uppercase text-[#F4F7F2]/80 font-mono font-medium tracking-[0.14em]">
                      <tr>
                    ...
                    <tbody className="divide-y divide-white/5 text-sm text-[#F4F7F2]/90">
                        ...
                        >
                          <td className="px-6 py-5">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                {user.full_name && (
                                  <p className="text-[0.65rem] text-[#B8C4B6]">{user.email}</p>
                                )}
                                <p className="text-[0.65rem] text-[#B8C4B6]/60">ID: {user.id.slice(0, 8)}…</p>
                              </div>
                            </div>
                          </td>
                          <td className="px-6 py-5">
                          ...
                          <td className="px-6 py-5">
                            <div className="flex flex-col gap-1.5">
                              <span className="text-[#D3DCD1]">
                                {user.manager_id ? (managerNameMap[user.manager_id] ?? "—") : "—"}
                              </span>
                              <button
                                onClick={() => setManagerModalUser(user)}
                                className="w-fit px-3 py-1.5 rounded-xl border border-[#E4EAE1]/25 text-xs font-semibold text-[#F4F7F2] hover:bg-white/5 transition-colors"
                              >
                                {user.manager_id ? "Change" : "Set manager"}
                              </button>
                            </div>
                            ...
                            {(() => {
                              ...
                              return (
                                <div className="flex flex-col gap-1.5">
                                  ...
                                  {!tenure && !user.experience_level && (
                                    <span className="text-xs text-[#B8C4B6]/60">Not set</span>
                                  )}
                                </div>
                              );
                            })()}
                            ...
                            {editingRoleUserId === user.id ? (
                              <div className="flex items-center gap-2" data-testid="edit-user-form">
                                ...
                                <select
                                  ...
                                  disabled={savingRole}
                                  className="rounded-xl bg-[#040605]/70 border border-[#F4F7F2]/20 px-3 py-1.5 text-xs text-[#F4F7F2] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#F4F7F2]/60"
                                >
                                  <option value="employee">Employee</option>
                                  <option value="admin">Admin</option>
                                  <option value="manager">Manager</option>
                                ...
                                <button
                                  ...
                                  disabled={savingRole}
                                  className="px-3 py-1.5 rounded-xl bg-[#F4F7F2]/20 border border-[#F4F7F2]/40 text-xs font-semibold text-[#F4F7F2] hover:bg-[#F4F7F2]/30 disabled:opacity-50 transition-colors whitespace-nowrap"
                                >
                                  {savingRole ? 'Saving...' : 'Save'}
                                </button>
                                ...
                                <button
                                  ...
                                  disabled={savingRole}
                                  className="px-3 py-1.5 rounded-xl border border-[#E4EAE1]/25 text-xs font-semibold text-[#F4F7F2] hover:bg-white/5 disabled:opacity-50 transition-colors"
                                >
                                  Cancel
                                </button>
                              </div>
                              ...
                              <div className="flex flex-wrap items-center gap-2">
                                <button
                                  ...
                                  className="px-3 py-1.5 rounded-xl border border-[#E4EAE1]/25 text-xs font-semibold text-[#F4F7F2] hover:bg-white/5 transition-colors"
                                >
                                  Edit Role
                                </button>
                                <button
                ...
                <div className="border-t border-[#E4EAE1]/15 bg-[#0B100D]/80">
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-3 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm text-[#D3DCD1]">
                    <div className="text-[10px] sm:text-sm">
                      <span className="text-[#F4F7F2]">{(currentPage - 1) * pageSize + 1}</span> –
                      <span className="text-[#F4F7F2]">
                        {" "}
                        {Math.min(currentPage * pageSize, totalUsers || 0)}
                      </span>{" "}
                      of
                      <span className="text-[#F4F7F2]"> {totalUsers || 0}</span> users
                    </div>
